...
class GetAll:
    ...
    def delete_enquiry(self):
        ...
        """
        try:
            id = str(readyaml(file=r'Door\enquiry', key='list_id'))
            url = Url + r'/manager/gwforward/manager-webapi/interaction/enquiry/deleteEnquiry?' + "&id="+id + "&isDelete=true"
            r = requests.get(url, headers=readconfig_ini(v=2), data={}, stream=True, verify=False)
            self.result = r.json()
