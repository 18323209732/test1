...
from Common.Template import case_data, case_body, class_title, pypublic, publicyaml, case_body_ddt
        ...
        if not self.pyfun:
            for fun in data['funName']:
                for funname in self.yafun:
                    if isinstance(fun[funname]['bar'], list):
                        strfun += case_body_ddt.format(funname, fun[funname]['case_name'], fun[funname]['mode'],
                                                     fun[funname]['re_bar'])
                    else:
